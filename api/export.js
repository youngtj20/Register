...
export default async function handler(req, res) {
  ...
  try {
    ...
    const headers = ['Registration ID', 'Full name', 'Phone', 'Email', 'Union/Organisation', 'Attendance', 'Check-in date/time', 'Method', 'Assisted by'];
    const rows = attendees.map((item) => `<Row>${cell(item.registration_code)}${cell(item.full_name)}${cell(item.phone)}${cell(item.email || '')}${cell(item.organisation || '')}${cell(item.checked_in ? 'Present' : 'Not present')}${cell(item.checked_in_at || '')}${cell(item.checkin_method || '')}${cell(item.checked_in_by_name || '')}</Row>`).join('');
