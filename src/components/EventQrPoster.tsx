...
function PosterCard({ event, checkinUrl, canvasRef }: { event: EventRecord; checkinUrl: string; canvasRef?: React.Ref<HTMLCanvasElement> }) {
  return <div className="qr-poster-print overflow-hidden rounded-[28px] border border-[#dfe8e3] bg-white p-6 text-center shadow-[0_30px_80px_rgba(5,29,21,.25)] sm:p-10">
  ...
  return <>
    <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <motion.div initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10 }} className="w-full max-w-lg">
        ...
        <div className="mb-3 flex items-center justify-center gap-2 rounded-full bg-[#073d2e] px-4 py-2 text-xs font-bold text-emerald-100 shadow-[0_10px_30px_rgba(6,61,46,.18)]">
        ...
        <PosterCard event={event} checkinUrl={checkinUrl} canvasRef={canvasRef} />
      </motion.div>
    </motion.div>
    <PrintPortal><PosterCard event={event} checkinUrl={checkinUrl} /></PrintPortal>
  </>;
}
