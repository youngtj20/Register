...
export default function MarkPresentConfirm({ attendee, busy, onCancel, onConfirm }: {
  attendee: Attendee;
  busy: boolean;
  onCancel: () => void;
  onConfirm: () => void;
  ...
  return <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
    <motion.div initial={{ opacity: 0, y: 24, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12 }} className="w-full max-w-sm overflow-hidden rounded-[28px] bg-white shadow-[0_30px_80px_rgba(5,29,21,.28)]">
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0b5b46] to-[#052c22] px-6 pb-8 pt-6 text-center text-white">
        ...
        <p className="relative mt-1 text-sm text-emerald-100/70">{attendee.phone}{attendee.organisation ? ` · ${attendee.organisation}` : ''}</p>
      </div>
      <div className="p-6 text-center sm:p-7">
        <h3 className="font-display text-base font-bold text-[#17201d]">Confirm check-in</h3>
        <p className="mx-auto mt-1.5 max-w-[260px] text-xs leading-5 text-[#6c7873]">Mark this guest as present at the reception desk.</p>
        <div className="mt-5 flex gap-2">
          <button type="button" className="btn-secondary flex-1 justify-center" onClick={onCancel} disabled={busy}>Cancel</button>
          <button type="button" className="btn-primary flex-1 justify-center" disabled={busy} onClick={onConfirm}>{busy ? <span className="loader-sm" /> : <Check size={16} />} Mark present</button>
        </div>
      </div>
    </motion.div>
  </motion.div>;
